...
/*
V2와 다른점은 SecretKey secretKey 멤버필드의 final 유무가 차이점.
ObjectMapper, AppProperties를 생성자를 통해 DI를 받고 있음.
SecretKey은 생성자 호출 이후에 @PostConstruct애노테이션을 가지고 있는 init 메소드를 통해 초기화 됨.
init는 메소드이지 생성자가 아니기 때문에 final을 초기화할 수 없다.
